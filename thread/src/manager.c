...
static void fill_pool(managed_pool_t *const pool) {
...
managed_pool_t* init_pool(const physics_t ex, const size_t n_threads, const grid_t grid) {
    ...
    pthread_barrier_t *start_barrier = NULL, *end_barrier = NULL;
    pthread_t *const tids = calloc(n_threads, sizeof(pthread_t));
    start_barrier = calloc(1, sizeof(pthread_barrier_t));
    end_barrier = calloc(1, sizeof(pthread_barrier_t));

    pthread_attr_t * worker_attrs = calloc(1, sizeof(pthread_attr_t));
    physics_t *const ex_allocated = calloc(1, sizeof(physics_t));

    size_t * rows_start = calloc(n_threads, sizeof(size_t));
    size_t * rows_end = calloc(n_threads, sizeof(size_t));
...
void await_workers(const managed_pool_t pool) {
    for (size_t i = 0; i < pool.n_threads; ++i) {
        void **retval = NULL;
        int joined = 0;
        if ((joined = pthread_join(pool.tids[i], retval))) {
            fprintf(stderr, "Failed to join with thread %lu: Error: %d\n", pool.tids[i], joined);
            exit(EXIT_FAILURE);
        }
...
int spawn_workers(const managed_pool_t pool, grid_t *const current_grid, grid_t *const prev_grid) {
    ...
    const size_t height = current_grid->h;
    ...
    for (size_t i = 0; i < pool.n_threads; ++i) {
        ...
        worker_args_t args_template = {
            ...
            .quit = pool.quit,
            .setup = pool.ex,
            .row_start = rows_per_worker * i,
            .row_end = rows_per_worker * (i + 1)
        };
